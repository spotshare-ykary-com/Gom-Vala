                           ...
                           gpointer    user_data)
{
   ...
   repository = GOM_REPOSITORY(g_task_get_source_object (task));
   version = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(task), "version"));
   migrator = g_object_get_data(G_OBJECT(task), "migrator");
   migrate_data = g_object_get_data(G_OBJECT(task), "migrator_data");
                             ...
                             GError                **error)
{
   ...
   task = g_task_new (G_OBJECT(repository), NULL, NULL, NULL);
   g_task_set_source_tag (task, gom_repository_migrate_sync);
   g_object_set_data(G_OBJECT(task), "version", GINT_TO_POINTER(version));
   g_object_set_data(G_OBJECT(task), "migrator", migrator);
   g_object_set_data(G_OBJECT(task), "migrator_data", migrator_data);
                              ...
                              gpointer               user_data)
{
   ...
   task = g_task_new (G_OBJECT(repository), NULL, callback, user_data);
   g_task_set_source_tag (task, gom_repository_migrate_async);
   g_object_set_data (G_OBJECT(task), "version", GINT_TO_POINTER (version));
   g_object_set_data (G_OBJECT(task), "migrator", migrator);
   g_object_set_data (G_OBJECT(task), "migrator_data", migrator_data);
                        ...
                        gpointer    user_data)
{
   ...
   repository = GOM_REPOSITORY(g_task_get_source_object(G_TASK (task)));
   g_assert(GOM_IS_REPOSITORY(repository));

   resource_type = (GType) g_object_get_data (G_OBJECT (task), "resource-type");
   g_assert(g_type_is_a(resource_type, GOM_TYPE_RESOURCE));

   filter = g_object_get_data (G_OBJECT (task), "filter");
   g_assert(!filter || GOM_IS_FILTER(filter));

   sorting = g_object_get_data (G_OBJECT (task), "sorting");
                                 ...
                                 GError        **error)
{
   ...
   task = g_task_new (G_OBJECT(repository), NULL, NULL, NULL);
   g_task_set_source_tag (task, gom_repository_find_sync);
   g_object_set_data(G_OBJECT(task), "resource-type",
                     GINT_TO_POINTER(resource_type));
   g_object_set_data_full(G_OBJECT(task), "filter",
                          filter ? g_object_ref(filter) : NULL,
                          filter ? g_object_unref : NULL);
   g_object_set_data_full(G_OBJECT(task), "sorting",
                          sorting ? g_object_ref(sorting) : NULL,
                          sorting ? g_object_unref : NULL);
   g_object_set_data(G_OBJECT(task), "queue", queue);

   gom_adapter_queue_read(priv->adapter, gom_repository_find_cb, task);
                                  ...
                                  gpointer             user_data)
{
   ...
   task = g_task_new (G_OBJECT(repository), NULL, callback, user_data);
   g_task_set_source_tag (task, gom_repository_find_sorted_async);
   g_object_set_data(G_OBJECT (task), "resource-type",
                     GINT_TO_POINTER (resource_type));
                            ...
                            gpointer      user_data)
{
   ...
   if (!gom_resource_group_get_count(group)) {
      g_task_return_new_error (task, GOM_ERROR,
                               ...
                               gpointer             user_data)
{
   ...
   task = g_task_new (G_OBJECT(repository), NULL, callback, user_data);
   g_task_set_source_tag (task, gom_repository_find_one_async);
   gom_repository_find_async (repository, resource_type, filter,
                              gom_repository_find_one_cb, task);
}
