...
gom_adapter_worker (gpointer data)
{
   ...
   while ((cmd = g_async_queue_pop (queue))) {
      ...
      g_object_unref(cmd->adapter);
      g_free(cmd);
      if (cmd->type == ASYNC_CMD_TYPE_CLOSE) break;
   }
               ...
               gpointer    user_data)
{
   ...
   g_task_return_boolean (task, ret == SQLITE_OK);
   if (queue)
                       ...
                       GError              **error)
{
   ...
   task = g_task_new (G_OBJECT(adapter), NULL, NULL, NULL);
   g_task_set_source_tag (task, gom_adapter_open_sync);
   g_object_set_data_full(G_OBJECT(task), "uri",
                          g_strdup(uri), g_free);
   g_object_set_data (G_OBJECT(task), "queue", queue);
   
   cmd = _async_cmd_new(adapter, ASYNC_CMD_TYPE_OPEN, open_callback, task);
                        ...
                        gpointer             user_data)
{
   ...
   task = g_task_new (G_OBJECT(adapter), NULL, callback, user_data);
   g_task_set_source_tag (task, gom_adapter_open_async);
                        ...
                        GError       **error)
{
   ...
   task = g_task_new (G_OBJECT(adapter), NULL, NULL, NULL);
   g_task_set_source_tag (task, gom_adapter_close_sync);
   g_object_set_data(G_OBJECT(task), "queue", queue);
   cmd = _async_cmd_new(adapter, ASYNC_CMD_TYPE_CLOSE, close_callback, task);
                         ...
                         gpointer             user_data)
{
   ...
   task = g_task_new (G_OBJECT(adapter), NULL, callback, user_data);
   g_task_set_source_tag (task, gom_adapter_close_async);
   ...
   cmd = _async_cmd_new (adapter, ASYNC_CMD_TYPE_CLOSE, open_callback, task);
