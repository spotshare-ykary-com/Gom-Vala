                        ...
                        gpointer    user_data)
{
   ...
   resource = GOM_RESOURCE(g_task_get_source_object(task));
   g_return_if_fail(GOM_IS_RESOURCE(resource));
                          ...
                          GError      **error)
{
   ...
   task = g_task_new (G_OBJECT(resource), NULL, NULL, NULL);
   g_task_set_source_tag (task, gom_resource_delete_sync);
   ...
   gom_adapter_queue_write(adapter, gom_resource_delete_cb, task);
                           ...
                           gpointer             user_data)
{
   ...
   task = g_task_new (G_OBJECT(resource), NULL, callback, user_data);
   g_task_set_source_tag (task, gom_resource_delete_async);
                        ...
                        GError      **error)
{
   ...
   queue = g_async_queue_new();
   task = g_task_new (G_OBJECT(resource), NULL, NULL, NULL);
   g_task_set_source_tag (task, gom_resource_save_sync);
   ...
   gom_adapter_queue_write(adapter, gom_resource_save_cb, task);
   g_async_queue_pop(queue);
   g_async_queue_unref(queue);
   ret = g_task_propagate_boolean (task, error);
   if (ret)
                         ...
                         gpointer             user_data)
{
   ...
   task = g_task_new (G_OBJECT(resource), NULL, callback, user_data);
   g_task_set_source_tag (task, gom_resource_save_async);
                           ...
                           gpointer    user_data)
{
   ...
   m2m_table = g_object_get_data(G_OBJECT(task), "m2m-table");
   ...
   if (!gom_cursor_next(cursor)) {
      g_task_return_new_error(task, GOM_ERROR, GOM_ERROR_RESOURCE_CURSOR,
                              ...
                              gpointer              user_data)
{
   ...
   g_assert(GOM_IS_ADAPTER(adapter));
   task = g_task_new (G_OBJECT(resource), NULL, callback, user_data);
   g_task_set_source_tag (task, gom_resource_fetch_m2m_async);
   g_object_set_data(G_OBJECT(task), "resource-type",
                     GINT_TO_POINTER(resource_type));
   g_object_set_data_full(G_OBJECT(task), "m2m-table",
                               ...
                               GError       **error)
{
   ...
   group = g_task_propagate_pointer (task, error);
