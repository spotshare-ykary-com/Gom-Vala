                             ...
                             gpointer    user_data)
{
   ...
   group = GOM_RESOURCE_GROUP(g_task_get_source_object (G_TASK (task)));

   g_assert (GOM_IS_ADAPTER (adapter));

   items = g_object_get_data (G_OBJECT (task), "items");
   queue = g_object_get_data (G_OBJECT (task), "queue");
                               ...
                               GError           **error)
{
   ...
   task = g_task_new (G_OBJECT(group), NULL, NULL, NULL);
   g_task_set_source_tag (task, gom_resource_group_write_sync);
   if (!group->priv->to_write)
      return TRUE;

   g_object_set_data(G_OBJECT(task), "queue", queue);
   items = group->priv->to_write;
   g_object_set_data_full(G_OBJECT(task), "items", items, (GDestroyNotify)g_ptr_array_unref);
   group->priv->to_write = NULL;

   adapter = gom_repository_get_adapter(group->priv->repository);
   gom_adapter_queue_write(adapter, gom_resource_group_write_cb, task);
                                ...
                                gpointer             user_data)
{
   ...
   task = g_task_new (G_OBJECT(group), NULL, callback, user_data);
   g_task_set_source_tag (task, gom_resource_group_write_async);
   if (!group->priv->to_write) {
      g_task_return_boolean (task, TRUE);
      return;
   }
   ...
   g_object_set_data_full(G_OBJECT(task), "items", group->priv->to_write, (GDestroyNotify)g_ptr_array_unref);
   group->priv->to_write = NULL;

   adapter = gom_repository_get_adapter(priv->repository);
   gom_adapter_queue_read(adapter, gom_resource_group_write_cb, task);
}
                                ...
                                GError           **error)
{
   ...
   task = g_task_new (G_OBJECT(group), NULL, NULL, NULL);
   g_task_set_source_tag (task, gom_resource_group_delete_sync);
   if (!group->priv->to_write)
      return TRUE;
   ...
   adapter = gom_repository_get_adapter(group->priv->repository);
   gom_adapter_queue_write(adapter, gom_resource_group_delete_cb, task);
   g_async_queue_pop(queue);
   g_async_queue_unref(queue);
                                 ...
                                 gpointer             user_data)
{
   ...
   task = g_task_new (G_OBJECT(group), NULL, callback, user_data);
   g_task_set_source_tag (task, gom_resource_group_delete_async);
   if (!group->priv->to_write) {
      g_task_return_boolean(task, TRUE);
      return;
   }
   ...
   adapter = gom_repository_get_adapter(priv->repository);
   gom_adapter_queue_read(adapter, gom_resource_group_delete_cb, task);
}
                             ...
                             gpointer    user_data)
{
   ...
   group = GOM_RESOURCE_GROUP(g_task_get_source_object(task));
   g_object_get(group,
                "filter", &filter,
                "sorting", &sorting,
                "m2m-table", &m2m_table,
   ...
   offset = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(task), "offset"));
   queue = g_object_get_data(G_OBJECT(task), "queue");
                               ...
                               GError           **error)
{
   ...
   queue = g_async_queue_new();
   task = g_task_new (G_OBJECT(group), NULL, NULL, NULL);
   g_task_set_source_tag (task, gom_resource_group_fetch_sync);
   
   g_object_set_data(G_OBJECT(task), "offset", GINT_TO_POINTER(index_));
   g_object_set_data(G_OBJECT(task), "limit", GINT_TO_POINTER(count));
   g_object_set_data(G_OBJECT(task), "queue", queue);


   adapter = gom_repository_get_adapter(group->priv->repository);
   gom_adapter_queue_read(adapter, gom_resource_group_fetch_cb, task);
   g_async_queue_pop(queue);
   g_async_queue_unref(queue);
   ret = g_task_propagate_boolean (task, error);
   g_object_unref (task);
                                ...
                                gpointer             user_data)
{
   ...
   priv = group->priv;
   task = g_task_new (G_OBJECT(group), NULL, callback, user_data);
   g_task_set_source_tag (task, gom_resource_group_fetch_async);
   g_object_set_data(G_OBJECT(task), "offset", GINT_TO_POINTER(index_));
   g_object_set_data(G_OBJECT(task), "limit", GINT_TO_POINTER(count));
                                 ...
                                 GError           **error)
{
   ...
   g_return_val_if_fail(GOM_IS_RESOURCE_GROUP(group), FALSE);
   g_return_val_if_fail(G_TASK (task), FALSE);
   g_return_val_if_fail(!group->priv->is_writable, FALSE);
   ret = g_task_propagate_boolean (task, error);
   g_object_unref (task);
